...
import 'package:nextflix/feature/data/repository/RepositoryImpl.dart';
import 'package:shared_preferences/shared_preferences.dart';
import '../../data/localdb/database/Database.dart';
import '../../data/localdb/entity/User.dart';
...
class NetflixBloc extends Bloc<NetflixEvent,NetflixState>{
  NetflixBloc(): super(NetflixInitial()){
    ...
    on<NavigateSearchEvent>(navigateSearchBloc);
    on<searchEvent>(searchBloc);
    on<OnBackPressedEvent>(backPressedEvent);
  }
  ...
  FutureOr<void> navigateSearchBloc(NavigateSearchEvent event, Emitter<NetflixState> emit) {
    emit(NavigateSearchScreenState());
  }



 /* FutureOr<void> searchBloc(searchEvent event, Emitter<NetflixState> emit) async{
    //var searchAns= await repository.searchMovie(event.name);
    var  configuration= await repository.getConfiguration();
    emit(searchListResult(searchAns.results,configuration));
  }*/
   ...
   FutureOr<void> searchBloc(searchEvent event, Emitter<NetflixState> emit) async{
     ...
     var searchAns= await repository.searchMovie(event.name);
    var  configuration= await repository.getConfiguration();
    print(searchAns.toString());
    emit(searchListResult(searchAns.results,configuration));
  }


  FutureOr<void> backPressedEvent(OnBackPressedEvent event, Emitter<NetflixState> emit) {
    emit(OnBackPressedState());
  }
}
